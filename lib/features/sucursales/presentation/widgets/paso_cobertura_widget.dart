...
// 🌟 Importamos nuestras "Muñecas Pequeñas" (Ajusta las rutas según tus carpetas)
...
class PasoCoberturaWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<EnvioAdminCubit, EnvioAdminState>(
      listener: (context, state) {
        if (state.mensajeExito != null) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(state.mensajeExito!), backgroundColor: Colors.green),
          );
          Navigator.of(context).pop(); // Salimos al guardar con éxito
        }
        if (state.error != null) {
          ScaffoldMessenger.of(context).showSnackBar(
      ...
      builder: (context, state) {
        return BlocBuilder<FormularioSucursalCubit, FormularioSucursalState>(
          builder: (context, formState) {
            final sucursalId = formState.sucursalIdCreada ?? sucursalOriginal?.id;

            if (sucursalId == null) {
              return const Padding(
                padding: EdgeInsets.all(16.0),
                child: Text('Error: No se encontró la sucursal generada en el Paso 1.'),
              );
            }
            ...
            return Column(
              ...
              children: [
                ...
                CoberturaSwitchWidget(
                  ...
                  cantidadSeleccionados: formState.codigosSeleccionados.length,
                  onChanged: (valor) {
                    context.read<EnvioAdminCubit>().alternarValidacionCP(valor);
                  },
                ),

                // ─── 2. ÁTOMO: CHIPS DE C.P. ───
                if (state.requiereCPTemporal)
                  SelectorChipsCpWidget(
                    ...
                    onToggle: (cp, seleccionado) {
                      context.read<FormularioSucursalCubit>().alternarCodigoPostal(cp, seleccionado);
                    },
                  ),
                ...
                // ─── INCENTIVO Y BOTÓN GUARDAR (Se quedan aquí por ser parte del formulario final) ───
                const Text('Promoción de Envío', style: TextStyle(fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                TextFormField(
                  ...
                  onChanged: (valor) => context.read<EnvioAdminCubit>().cambiarMontoGratis(double.tryParse(valor)),
                ),
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () {
                      ...
                      final configuracion = ConfiguracionEnvio(
                        ...
                        estrategia: state.configuracion?.estrategia ?? EstrategiaEnvio.hibrida,
                        ...
                        codigosPostalesPermitidos: formState.codigosSeleccionados,
                        activa: true,
                        fechaCreacion: state.configuracion?.fechaCreacion ?? DateTime.now(),
                      );
